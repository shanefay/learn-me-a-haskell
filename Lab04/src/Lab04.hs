...
execute04 hin hout "add" state =
  do hPutStr hout "Enter item description to add:\n"
     description_in <- hGetLine hin
     let description = (description_in :: String)

     hPutStr hout "Enter item value:\n"
     value_in <- hGetLine hin
     let value = read value_in :: Float

     let bt = add_to state description value
...
execute04 hin hout "remove" state =
   do hPutStr hout "Enter item description to remove:\n"
      description_in <- hGetLine hin
      let description = (description_in :: String)
      let val = treeLookup description (fst state)
      let value = fromJust val
      let datum = 0

      let bt = treeInsert description datum (fst state)
